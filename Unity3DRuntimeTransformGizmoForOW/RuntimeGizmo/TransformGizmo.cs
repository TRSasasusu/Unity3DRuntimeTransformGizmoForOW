...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		public Key SetMoveType = Key.W;
		public Key SetRotateType = Key.E;
		public Key SetScaleType = Key.R;
		//public Key SetRectToolType = Key.T;
		public Key SetAllTransformType = Key.Y;
		...
		void HandleUndoRedo()
		{
			...
			if (Keyboard.current[UndoAction].isPressed)
			{
				...
				if (Keyboard.current[UndoAction].wasPressedThisFrame)
				{
					UndoRedoManager.Undo();
				}
				//else if(Input.GetKeyDown(RedoAction))
				else if (Keyboard.current[RedoAction].wasPressedThisFrame)
				{
		...
		void SetSpaceAndType()
		{
			...
			if (Keyboard.current[ActionKey].isPressed) return;

			//if(Input.GetKeyDown(SetMoveType)) transformType = TransformType.Move;
			if (Keyboard.current[SetMoveType].wasPressedThisFrame) transformType = TransformType.Move;
			//else if(Input.GetKeyDown(SetRotateType)) transformType = TransformType.Rotate;
			else if(Keyboard.current[SetRotateType].wasPressedThisFrame) transformType = TransformType.Rotate;
			//else if(Input.GetKeyDown(SetScaleType)) transformType = TransformType.Scale;
			else if(Keyboard.current[SetScaleType].wasPressedThisFrame) transformType = TransformType.Scale;
			...
			else if(Keyboard.current[SetAllTransformType].wasPressedThisFrame) transformType = TransformType.All;

			if(!isTransforming) translatingType = transformType;

			//if(Input.GetKeyDown(SetPivotModeToggle))
			if(Keyboard.current[SetPivotModeToggle].wasPressedThisFrame)
			{
			...
			if(Keyboard.current[SetCenterTypeToggle].wasPressedThisFrame)
			{
			...
			if(Keyboard.current[SetSpaceToggle].wasPressedThisFrame)
			{
			...
			if(Keyboard.current[SetScaleTypeToggle].wasPressedThisFrame)
			{
		...
		IEnumerator TransformSelected(TransformType transType)
		{
			...
			while(!Mouse.current.leftButton.wasReleasedThisFrame)
			{
				...
				bool isSnapping = Keyboard.current[translationSnapping].isPressed;
				...
				if(previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					...
					else if(transType == TransformType.Rotate)
					{
						...
						if(nearAxis == Axis.Any)
						{
							...
							Vector3 rotation = transform.TransformDirection(new Vector3(Mouse.current.delta.y.ReadValue(), -Mouse.current.delta.x.ReadValue(), 0));
		...
		void GetTarget()
		{
			...
			if(nearAxis == Axis.None && Mouse.current.leftButton.wasPressedThisFrame)
			{
				...
				bool isAdding = Keyboard.current[AddSelection].isPressed;
				//bool isRemoving = Input.GetKey(RemoveSelection);
				bool isRemoving = Keyboard.current[RemoveSelection].isPressed;
		...
		void SetMaterial()
		{
			if(lineMaterial == null)
			{
				...
				lineMaterial = new Material(Unity3DRuntimeTransformGizmoForOW.Unity3DRuntimeTransformGizmoForOW._lineShader);
				outlineMaterial = new Material(Unity3DRuntimeTransformGizmoForOW.Unity3DRuntimeTransformGizmoForOW._outlineShader);
			}
		}
	}
}
